...
AA_BENCHMARK_FIELDS = {
    ...
    "gdpval_aa":            "bench_gdpval_aa",
    "terminal_bench_hard":  "bench_terminal_bench",
    "tau2_bench":           "bench_tau2_bench",
    "aa_lcr":               "bench_aa_lcr",
    "omniscience_accuracy": "bench_omniscience_acc",
    "omniscience_nonhallucination": "bench_omniscience_nohal",
    "hle":                  "bench_hle",
    "gpqa_diamond":         "bench_gpqa",
    "scicode":              "bench_scicode",
    "ifbench":              "bench_ifbench",
    "critpt":               "bench_critpt",
    "apex_agents_aa":       "bench_apex_agents",
    "mmmu_pro":             "bench_mmmu_pro",

    # Performance metrics
    "output_tokens_per_second":           "speed_output_tps",
    "median_time_to_first_token_seconds": "latency_ttft_s",
    "median_end_to_end_response_time_seconds_500_output_tokens": "latency_e2e_500_s",
}

# Fields we pull from AA's pricing/specs section
AA_SPEC_FIELDS = {
    "context_window":              "aa_context_window_k",   # in tokens, we convert to k
    "input_price_usd_per_1m":      "aa_price_input_per_1M",
    "output_price_usd_per_1m":     "aa_price_output_per_1M",
    "cache_read_price_usd_per_1m": "aa_price_cache_read_per_1M",
}
...
def extract_aa_fields(aa_model: dict) -> dict:
    """
    Extract the benchmark scores and performance metrics we care about
    from a raw AA API model response.

    Returns a flat dict with our internal field names.
    """
    result = {"aa_data": True}

    # The AA API may nest benchmarks under different keys
    # Common structures: {"evaluations": {...}}, {"benchmarks": {...}}, flat top-level
    evals = (
        aa_model.get("evaluations") or
        aa_model.get("benchmarks") or
        aa_model.get("intelligence_evaluations") or
        aa_model          # fall back to top-level
    )

    performance = (
        aa_model.get("performance") or
        aa_model.get("speed") or
        aa_model   # fall back to top-level
    )

    pricing = (
        aa_model.get("pricing") or
        aa_model.get("price") or
        aa_model
    )

    specs = (
        aa_model.get("specifications") or
        aa_model.get("specs") or
        aa_model
    )

    # Extract benchmark fields
    for aa_field, our_field in AA_BENCHMARK_FIELDS.items():
        # Try multiple possible nestings
        val = (
            evals.get(aa_field) or
            performance.get(aa_field) or
            aa_model.get(aa_field)
        )
        # AA sometimes uses None for "not tested" vs 0 for "scored zero"
        result[our_field] = float(val) if val is not None else None

    # Extract spec fields
    ctx = specs.get("context_window") or aa_model.get("context_window")
    if ctx:
        # Convert from tokens to k-tokens
        result["aa_context_window_k"] = int(ctx) // 1000

    for aa_field, our_field in AA_SPEC_FIELDS.items():
        if aa_field == "context_window":
            continue   # handled above
        val = pricing.get(aa_field) or specs.get(aa_field) or aa_model.get(aa_field)
        result[our_field] = float(val) if val is not None else None

    # AA intelligence index — sometimes top-level
    for key in ("intelligence_index", "aa_intelligence_index", "index_score"):
        val = aa_model.get(key)
        if val is not None:
            result["aa_intelligence_index"] = float(val)
            break
...
def main():
    ...
    parser.add_argument("--out",     default="model_registry.json",
                        help="Output path (default: model_registry.json)")
